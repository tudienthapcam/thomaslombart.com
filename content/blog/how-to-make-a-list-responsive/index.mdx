...
- How do you manage the gutter of your grid? Do you set a `margin` for all elements? If yes, how do you handle the last and first element's margins?
...
- `minmax(200px, 1fr)`: this function takes two arguments, a minimum and a maximum (what a surprise!). Here it means, the function will create columns with a minimum of `200px` and a maximum of `1fr`. An `fr` is a fractional unit that takes one part of the available space.
