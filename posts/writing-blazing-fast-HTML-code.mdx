...
When it comes to writing HTML code, it can get pretty tedious.

Let’s assume you want to write your items list. Well, this piece of code:
...
<Image
  src="/writing-blazing-fast-HTML-code/simple-list.png"
  alt="A simple item list"
/>
...
Emmet is a plugin for text editors that will allow you to be way more productive when writing HTML code. Basically, you write a snippet, and you press your tab key and Voilà. You just gained some time for the same result.

What if I said that the example above could be written in a single line? You don’t believe me? Here’s how:
...
<YouTubeEmbed
  id="3sad_66vGbc"
  title="YouTube video on ow to write super-fast HTML with Emmet"
/>
...
Same as the one above, instead that this time you’re creating an element with an ID.
...
Now this one is a real time-saver. Instead of formatting your HTML manually and spending time writing all your `>` and `<`, you use `>` to nest all your tags.
...
You can even choose which number you want to start with `@`:
...
Practice makes perfect! Here is some HTML code. Try to figure out what was the snippet. Don’t look now at the solution. Play around with Emmet. It’s the only way to really learn!
