...
As a prerequisite, we will need [Node.js](https://nodejs.org/en/) installed on our machines.
...
As seen before, creating a default project with Vue CLI comes with Babel and ESLint enabled by default. One important thing to note is that Vue CLI projects use [Webpack](https://webpack.js.org/): a module bundler.
...
`.browserslistrc`: this file allows us to define which browsers we want to target to **generate the optimal output files**. Here, the file targets all browser versions that have more than 1% of market shares (`> 1%`) and the last two versions of each browser (`last 2 versions`). You can visualize them on [browserl.ist](https://browserl.ist/?q=%3E+1%25%2C+last+2+versions).
...
Look at the `App.vue` file. If you already [played around with Vue.js](https://thomaslombart.com/getting-started-with-vue/) you may have defined your Vue instance like this:
